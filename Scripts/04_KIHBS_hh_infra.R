...
# For water, there may be a 30 min threshold that we calculate for
...
hh_wash <- hh_inf %>% 
  ...
  mutate(improved_water_drink = case_when(
    ...
    impr_water_under30min = ifelse(is.na(water_time) | is.na(improved_water_drink), NA,
                                   ifelse(water_time <= 30  & improved_water_drink == 1, 1, 0)), 
  
  # Now add in sanitation variables
 # -- straight classification of whether the source is improved --
...
hh_wash %>% group_by(toilet_type, improved_toilet) %>% summarise(n = n()) %>% ungroup() %>% mutate(pct = percent(n/sum(n), ndigits = 1))

hh_wash %>% group_by(impr_toilet_type) %>% summarise(n = n()) %>%  mutate(pct = percent(n/sum(n), ndigits = 1))
...
hh_wash_base %>% 
  ...
  scale_fill_viridis_c(alpha = 0.75, option = "D", direction = -1)
...
hh_wash_svy %>% summarise(survey_mean(improved_water_drink_bot, na.rm = TRUE))
