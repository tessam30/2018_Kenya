...
# Another option for colors from CartoDB -- #855C75,#D9AF6B,#AF6458,#736F4C,#526A83,#625377,#68855C,#9C9C5E,#A06177,#8C785D,#467378,#7C7C7C ; Prism colors
 cat_colrs = c("#5F4690", "#1D6996", "#38A6A5",
...
budget %>% 
  ...
  scale_fill_viridis_c(direction = -1, alpha = 0.90, 
                       option = "A", label = percent_format(accuracy = 2)) + # format labels in legend
  theme_minimal() +  
  ...
  theme(legend.position = "top",
        ...
        panel.spacing = unit(1.5, "lines")) + # adding more spaced between panels
  labs(fill = 'Share of Development Expenditures', 
       y = "", x = "",
       caption = GC_caption) +
